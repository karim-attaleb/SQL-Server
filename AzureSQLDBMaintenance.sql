CREATE PROCEDURE [AzureSQLDBMaintenance]
	(
...
begin
	...
	begin
		...
		raiserror('     "no_limit" going through all indexes and statistics regardless thier modifications or fragmentation.',0,0)
		raiserror(' ',0,0)
		...
		raiserror('     1 - Rebuild heaps based on @mode parameter, @mode=no_limit will rebuild all heaps',0,0)
		raiserror(' ',0,0)
		...
		raiserror('		exec  AzureSQLDBMaintenance ''all'', @LogToTable=1',0,0)
	end
	else 
	begin
		...
		---------------------------------------------
		/* Prepare Log Table */
		if object_id('AzureCommandLog') is null and @LogToTable=1
		begin
			create table AzureCommandLog (id bigint primary key identity(1,1), OperationTime datetime2, command varchar(4000),ExtraInfo varchar(4000), StartTime datetime2, EndTime datetime2, StatusMessage varchar(1000));
		end
		...
		if OBJECT_ID('AzureCommandLogQueue') is not null 
		begin
			if 
				/*resume information exists*/ exists(select * from AzureCommandLogQueue where ID=-1) 
			begin
				...
				begin
					-- table [AzureCommandLogQueue] exist but resume information does not exists
					...
					DROP TABLE [AzureCommandLogQueue];
				end
		end
	...
	if @LogToTable=1 insert into AzureCommandLog values(@OperationTime,null,null,sysdatetime(),sysdatetime(),'Starting operation: Operation=' +@operation + ' Mode=' + @mode + ' Keep log for last ' + cast(@KeepXOperationInLog as varchar(10)) + ' operations' )	

	-- create command queue table, if there table exits then we resume operation in earlier stage.
	if @operation!='resume'
		create table AzureCommandLogQueue (ID int identity primary key,txtCMD nvarchar(max),ExtraInfo varchar(max))
	...
	begin
		...
		begin 
				...
				raiserror(@msg,0,0)
				if @LogToTable=1 insert into AzureCommandLog values(@OperationTime,null,null,sysdatetime(),sysdatetime(),@msg)	
		end
	end
	...
	begin
		...
		where (
				...
				and @mode != 'no_limit' /*for no_limit mode we do not want to skip anything */
		...
		update #idxBefore set OperationToTake = 
			...
			(
				...
				or 
				(@mode='no_limit' and type in (5,6))/* Columnstore indexes in no_limit mode -> reorganize them */
			then
				 'REORGANIZE'
			else 
				'REBUILD'
		...
		*/
		...
		insert into AzureCommandLogQueue(txtCMD,ExtraInfo)
		select 
		...
		, ExtraInfo =
			...
			case when type in (5,6) then
				'no_limit mode therefore reorganize columnstore indexes'
			else 
	...
	begin 
		...
		where OBJECT_SCHEMA_NAME(s.object_id) != 'sys' and /*Modified stats or no_limit mode*/(isnull(sp.modification_counter,0)>0 or @mode='no_limit')
	...
	begin
		...
		begin
			...
			if @debug='whatif' 
			begin
				update AzureCommandLogQueue set txtCMD = '--' + txtCMD
			end
			...
			if object_id('AzureCommandLogQueue') is not null select * into cmdQueue from AzureCommandLogQueue
		end
		...
		begin
			...
			set identity_insert AzureCommandLogQueue on
			insert into AzureCommandLogQueue(ID,txtCMD,ExtraInfo) values(-1,'parameters to be used by resume code path',@ExtraInfo)
			set identity_insert AzureCommandLogQueue off
		end
		...
		*/
		...
		while exists(select * from AzureCommandLogQueue where ID>0)
		begin
			update top (1) AzureCommandLogQueue set txtCMD=txtCMD output deleted.* into @T where ID>0;
			select top (1) @ID = ID, @SQLCMD = txtCMD, @ExtraInfo=ExtraInfo from @T
			raiserror(@SQLCMD,0,0) with nowait
			if @LogToTable=1 insert into AzureCommandLog values(@OperationTime,@SQLCMD,@ExtraInfo,sysdatetime(),null,'Started')
			begin try
				exec(@SQLCMD)	
				if @LogToTable=1 update AzureCommandLog set EndTime = sysdatetime(), StatusMessage = 'Succeeded' where id=SCOPE_IDENTITY()
			end try
			begin catch
				set @ScriptHasAnError=1;
				set @msg = 'FAILED : ' + CAST(ERROR_NUMBER() AS VARCHAR(50)) + ERROR_MESSAGE();
				raiserror(@msg,0,0) with nowait
				if @LogToTable=1 update AzureCommandLog set EndTime = sysdatetime(), StatusMessage = @msg where id=SCOPE_IDENTITY()
			end catch
			delete from AzureCommandLogQueue where ID = @ID;
			delete from @T
		end
		drop table AzureCommandLogQueue;
	end
	...
	if @LogToTable=1
	begin
		delete from AzureCommandLog 
		from 
			AzureCommandLog L join 
			(select distinct OperationTime from AzureCommandLog order by OperationTime desc offset @KeepXOperationInLog rows) F
				ON L.OperationTime = F.OperationTime
		insert into AzureCommandLog values(@OperationTime,null,cast(@@rowcount as varchar(100))+ ' rows purged from log table because number of operations to keep is set to: ' + cast( @KeepXOperationInLog as varchar(100)),sysdatetime(),sysdatetime(),'Cleanup Log Table')
	end

	if @ScriptHasAnError=0 	raiserror('Done',0,0)
	if @LogToTable=1 insert into AzureCommandLog values(@OperationTime,null,null,sysdatetime(),sysdatetime(),'End of operation')
	if @ScriptHasAnError=1 	raiserror('Script has errors - please review the log.',16,1)
end
GO
print 'Execute AzureSQLDBMaintenance to get help' 
