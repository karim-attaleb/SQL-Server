...
.EXAMPLE
    .\Create-Database.ps1 -SqlInstance "S2S005G2\POD07_DEV" -Database "DB_MSS0_DEMO" -DataDrive "G,H" -LogDrive "E"
.EXAMPLE
    .\Create-Database.ps1 -SqlInstance "S2S005G2\POD07_DEV" -Database "DB_MSS0_DEMO" -DataDrive "G" -LogDrive "E" -NumberOfDataFiles 4
#>
...
try {
    ...
    try {
        ...
        Write-Error "dbatools module is required. Please install with: Install-Module dbatools"
        exit 1
    }
    ...
    foreach ($login in $loginsToCreate) {
        ...
        if (-not $existingLogin) {
            try {
                ...
                Write-Log -Message "Failed to create login $($login.Name): $($_.Exception.Message)" -Level Error
            }
        }
        else {
